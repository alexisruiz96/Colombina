...
//  Copyright � 2018 Oriol Martinez Pujol. All rights reserved.
//
...
Tmplt::Tmplt(int patchsz, int descsz) {
	...
	hog = new HOGDescriptor(cvSize(Wsz,Wsz),cvSize(Bsz,Bsz),cvSize(Bst,Bst),cvSize(Csz,Csz),nbins,1,Ws,0,0.2,true,64,true);
...
void Tmplt::detectFaces(cv::Mat grayframe, vector<Rect>& faces) {
	...
	fd.detectMultiScale(grayframe, faces, 1.2, 2, 0, cvSize(50, 50));
...
float Tmplt::estimateLandmarks(cv::Mat patch, std::vector<cv::Point2f>& esPts, vector<cv::Mat> omega) {
	...
	for (int i = 0; i < omega.size(); i++) {
		...
		//OpenCV HOG
		hog->compute(patch, descriptors, cv::Size(), cv::Size(), selPts);
		cv::vconcat(Mat::ones(1, 1, CV_32FC1), Mat::Mat(descriptors),phi);
